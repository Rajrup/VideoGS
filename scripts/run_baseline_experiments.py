...
def run_dracogs(sequence: str, selected_frames: list[int], dry_run: bool, skip_existing: bool) -> None:
    ...
    if skip_existing and _output_complete(paths.output_folder):
        log_step(f"SKIP (exists) DracoGS | {sequence} | frames: {frame_start}-{frame_end - 1}:{interval}")
        return
    log_step(
...
def run_mesongs(sequence: str, selected_frames: list[int], dry_run: bool, skip_existing: bool) -> None:
    ...
    if skip_existing and _output_complete(paths.output_folder):
        log_step(f"SKIP (exists) MesonGS | {sequence} | frames: {frame_start}-{frame_end - 1}:{interval}")
        return
    log_step(
        ...
        for qp in VIDEOGS_QPS:
            ...
            if skip_existing and _output_complete(paths.output_folder):
                log_step(
                    ...
                    f"anchor: {anchor_frame} | frames: {gop_start}-{gop_end - 1}:1"
                )
                continue
            log_step(
