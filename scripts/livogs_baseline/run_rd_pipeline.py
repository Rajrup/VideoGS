#!/usr/bin/env python3
"""High-level orchestrator for the LiVoGS per-frame RD experiment pipeline.

Stages (each can be toggled on/off):
  1. GENERATE  — call qp.generate() to produce per-channel QP JSON files
  2. EVALUATE  — spawn worker.py (one subprocess per GPU job) for every QP config
  3. PLOT      — call plot.main() to produce RD-curve figures
...
# ---------------------------------------------------------------------------
# Global configuration  (edit here — this is the SINGLE source of truth)
# ---------------------------------------------------------------------------
...
#    - "curve_var" is the x-axis grouping variable for separate RD curves.
#    - "fixed" contains exact-match filters applied before plotting.
# 2) Aggregation depth selection in main():
#    - If curve_var == "depth", we aggregate all EXPERIMENT_DEPTHS.
#    - Otherwise, if fixed contains "depth", we aggregate only that depth.
#    - If fixed omits "depth", we aggregate all EXPERIMENT_DEPTHS.
#
...
class Stage2Job:
    ...
    def __init__(self, idx: int, label: str, depth: int, gpu_id: int,
...
def _run_subprocess(label: str, cmd: list[str], cwd: str = config.VIDEOGS_ROOT,
...
def _format_config_key_list(keys: set[ConfigKey], max_items: int = 6) -> str:
    ...
    summary = ", ".join([f"(shqp={s}, β={b}, q={q}, s={sc}, o={o})" for s, b, q, sc, o in shown])
...
def stage_generate(
    ...
    """Stage 1: generate per-channel QP config JSONs (direct call, no subprocess)."""
    sep = "=" * 70
...
def ensure_experiment_configs() -> bool:
    ...
    if unresolved:
        ...
        for seq, frame_id, missing, expected_count, existing_count, invalid in unresolved:
            ...
            print(f"    Missing config keys: {_format_config_key_list(missing)}")
        return False
...
def _is_saved_experiment_complete(
    seq: SequenceCfg, frame_id: int, depth: int, label: str,
...
def stage_evaluate(seq: SequenceCfg, frame_id: int, depths: list[int]) -> list[str]:
    """Stage 2: compress + evaluate for every QP config JSON.

    Spawns ``worker.py`` subprocesses — one per GPU job.
    Returns list of labels that failed.
    """
    json_files = find_qp_jsons(seq, frame_id)
    ...
    for json_path in json_files:
        ...
        for depth in depths:
            if SKIP_SAVED_EXPERIMENTS and _is_saved_experiment_complete(
                seq=seq, frame_id=frame_id, depth=depth, label=label,
            ...
            jobs.append(Stage2Job(
                idx=idx, label=label, depth=depth, gpu_id=gpu_id,
                cmd=cmd, env=env,
            ))
...
def stage_plot(
    ...
    if curve_var not in config.KNOB_NAMES:
        print(f"[WARN] Invalid curve_var '{curve_var}' in plot spec. Expected one of {sorted(config.KNOB_NAMES)}")
        return

    invalid_fixed_keys = [key for key in fixed.keys() if key not in config.KNOB_NAMES]
    if invalid_fixed_keys:
        print(f"[WARN] Invalid fixed keys {invalid_fixed_keys} in plot spec; skipping.")
        return
...
def _depths_needed_for_plotting(
    ...
    Rules:
    - Any spec with curve_var="depth" requires the full experiment depth list.
    - Specs that fix depth (fixed["depth"]) request that specific depth.
    - If no spec fixes depth, fall back to full experiment depths.
    """
