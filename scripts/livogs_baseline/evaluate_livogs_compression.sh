...
output_folder="${data_path}/train_output/${DATASET_NAME}/${SEQUENCE_NAME}/livogs_compression/J_${J}_qstep_${QUANTIZE_STEP}_${SH_COLOR_SPACE}"

### 1. LiVoGS Compress + Decompress (encode → bytestream on GPU → decode → save PLY)
echo "======================================================================"
echo "Step 1: LiVoGS Compress + Decompress"
echo "======================================================================"
python compress_livogs/compress_decompress_livogs.py \
...
echo "======================================================================"
python compress_livogs/evaluate_decompression_quality.py \
    --gt_ply_path "${gt_model_path}" \
    --decompressed_ply_path "${output_folder}/decompressed_ply" \
    --dataset_path "${dataset_path}" \
    --output_render_path "${output_folder}/evaluation_renders" \
    --save_renders \
    --sh_degree ${SH_DEGREE} \
