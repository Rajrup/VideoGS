...
SCATTER_SPEC: dict[str, Any] = {
    ...
    "fixed": {
        "depth": [8,9,10,11,12],
        "qp_sh": [v / 255.0 for v in (1,2,4,8,16,32,64,128)],
        "qp_opacity": [0.01, 0.1, 0.5],
        "qp_scales": [0.0001, 0.001, 0.002],
        "qp_quats": [0.001, 0.005, 0.01, 0.03, 0.05],
    },
}
...
def _infer_dataset_name(rd_root: str) -> str:
    ...
    for i, part in enumerate(parts):
        if part == "train_output" and i + 1 < len(parts):
            return parts[i + 1]
...
def main() -> None:
    sep = "=" * 70
    print(sep)
    print("LiVoGS RD Hull Plot Pipeline (VideoGS)")
