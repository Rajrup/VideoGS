...
load_videogs_ply = pipeline.load_videogs_ply
...
FORMAT_DEFAULTS = {
    "queen": {"J": 17, "sh_degree": 2, "frame_start": 1, "frame_end": 300},
    "videogs": {"J": 12, "sh_degree": 3, "frame_start": 0, "frame_end": 200},
}
...
def main():
    ...
    p.add_argument("--format", type=str, default="videogs", choices=["queen", "videogs"])
    ...
    if args.sh_degree is None:
        args.sh_degree = FORMAT_DEFAULTS[args.format]["sh_degree"]
    if args.frame_start is None:
        args.frame_start = FORMAT_DEFAULTS[args.format]["frame_start"]
    if args.frame_end is None:
        args.frame_end = FORMAT_DEFAULTS[args.format]["frame_end"]

    if args.format == "queen" and (
        load_queen_ply is None or find_queen_ply_path is None
    ):
        raise RuntimeError(
    ...
    if args.format == "queen":
        assert queen_find is not None
        assert queen_load is not None

    def _load_frame(frame_id):
        if args.format == "queen":
            if queen_find is None or queen_load is None:
            ...
            if ply_file is None:
                return None, None
            params, _ = queen_load(ply_file, device=device)
            return params, ply_file

        ply_file = find_videogs_ply_path(args.ply_path, frame_id)
        if ply_file is None:
            return None, None
        params, _ = load_videogs_ply(ply_file, device=device)
        return params, ply_file

    # --- Warmup ---
    print("Warming up GPU...")
    frame0 = args.frame_start
    params, _ = _load_frame(frame0)
    if params is None:
        raise ValueError(f"PLY not found for warmup frame {frame0} in {args.ply_path}")
    ...
    for frame in tqdm(frames, desc="Frames"):
        params, ply_file = _load_frame(frame)
        if params is None:
            print(f"  WARNING: PLY not found for frame {frame} in {args.ply_path}, skipping")
            continue
