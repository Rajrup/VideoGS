...
python compress/videogs/compress_to_png_full_sh.py --frame_start ${START_FRAME} --frame_end ${END_FRAME} --group_size ${GROUP_SIZE} --interval ${INTERVAL} --ply_path ${gt_model_path} --output_folder "${output_folder}/compressed_png" --sh_degree ${SH_DEGREE}

### 2. Compress PNG to MP4 (H.264)
echo "Compressing PNG to MP4 (H.264)..."
python compress/videogs/compress_png_2_video.py --input_folder "${output_folder}/compressed_png" --output_folder "${output_folder}/compressed_video" --qp ${QP} --sh_degree ${SH_DEGREE}

### 3. Decompress MP4 to PNG
echo "Decompressing MP4 to PNG..."
python compress/videogs/decompress_video_2_png.py --input_folder "${output_folder}/compressed_video" --output_folder "${output_folder}/decompressed_png"

### 4. Decompress PNG to PLY (Full SH)
echo "Decompressing PNG to PLY (Full SH)..."
python compress/videogs/decompress_from_png_full_sh.py --compressed_folder "${output_folder}/decompressed_png" --output_ply_folder "${output_folder}/decompressed_ply" --sh_degree ${SH_DEGREE}
...
echo "======================================================================"
python compress/evaluate_decompress.py --gt_ply_path ${gt_model_path} --decompressed_ply_path "${output_folder}/decompressed_ply" --dataset_path ${dataset_path} --output_render_path "${output_folder}/evaluation" --save_renders --sh_degree ${SH_DEGREE} --resolution ${RESOLUTION} --frame_start ${START_FRAME} --frame_end ${END_FRAME} --interval ${INTERVAL}
